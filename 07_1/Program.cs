...
wkvfo (177) -> muaiipe, nctisdd, tdyqxhp";
		...
		{
			var allProgramNames = new HashSet<string>();
			var referencedProgramNames = new HashSet<string>();

			using (var reader = new StringReader(Input))
			{
				var parser = new Regex(@"^(?<ProgramName>\w+) \(\d+\)( -\> (?<ReferredTo>\w+)(, (?<ReferredTo>\w+))*)?$", RegexOptions.Singleline);
				...
				while (true)
				{
					...
					allProgramNames.Add(match.Groups["ProgramName"].Value);
					referencedProgramNames.UnionWith(match.Groups["ReferredTo"].Captures.OfType<Capture>().Select(cap => cap.Value));
				}
			}

			var unreferenced = allProgramNames.Except(referencedProgramNames).ToList();

			Console.WriteLine(unreferenced.Count);

			foreach (var programName in unreferenced)
