...
p=<-2954,-985,167>, v=<-418,-140,23>, a=<29,7,-3>";
		...
		class Particle
		{
			...
			public static Particle Parse(string str)
			{
				...
				if (!match.Success)
					throw new FormatException("Can't parse: " + str);

				var particle = new Particle();

				particle.Position.X = long.Parse(match.Groups["PX"].Value);
				particle.Position.Y = long.Parse(match.Groups["PY"].Value);
				particle.Position.Z = long.Parse(match.Groups["PZ"].Value);

				particle.Velocity.X = long.Parse(match.Groups["VX"].Value);
				particle.Velocity.Y = long.Parse(match.Groups["VY"].Value);
				particle.Velocity.Z = long.Parse(match.Groups["VZ"].Value);

				particle.Acceleration.X = long.Parse(match.Groups["AX"].Value);
				particle.Acceleration.Y = long.Parse(match.Groups["AY"].Value);
				particle.Acceleration.Z = long.Parse(match.Groups["AZ"].Value);
			...
			public bool IsResolved
			{
				get
				{
					...
					return
						IsResolved(Position.X, Velocity.X) &&
						IsResolved(Position.Z, Velocity.Z) &&
						IsResolved(Position.Y, Velocity.Y) &&
						IsResolved(Acceleration.X, Velocity.X) &&
						IsResolved(Acceleration.Y, Velocity.Y) &&
						IsResolved(Acceleration.Z, Velocity.Z);
				}
			}
		...
		static void Main(string[] args)
		{
			...
			Console.WriteLine("{0} particles", particles.Count);

			int closestIndex = -1;
			long closestDistance = long.MaxValue;

			// Couldn't be bothered to figure out how to check whether we need to do more iterations or not. Would have taken longer than running these iterations takes.
			// Technically, the input *could* have particles that switch places in their relative Manhattan distances to origin after 100,000 iterations...
			for (int iteration = 0; iteration < 100000; iteration++)
			{
				...
				for (int i = 0; i < particles.Count; i++)
				{
					...
					var thisParticleDistanceToOrigin = particle.ManhattanDistanceToOrigin;

					if (thisParticleDistanceToOrigin < closestDistance)
					{
						closestDistance = thisParticleDistanceToOrigin;
						closestIndex = i;

						Console.WriteLine("Closest index changed to {0} on iteration {1}", closestIndex, iteration);
					}
