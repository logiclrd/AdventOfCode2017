...
ax dec 329 if ga <= 3826";
		...
		static void Main(string[] args)
		{
			...
			int maximumEver = int.MinValue;

			int GetRegister(string name)
			{
				int value;

				registers.TryGetValue(name, out value);
			...
			{
				var parser = new Regex(@"^(?<AlterRegisterName>\w+) (?<Operation>inc|dec) (?<Operand>-?\d+) if (?<CompareRegisterName>\w+) (?<Comparison>\>|\<|\>=|\<=|==|!=) (?<Comparand>-?\d+)$");
				...
				while (true)
				{
					...
					string alterRegisterName = match.Groups["AlterRegisterName"].Value;
					string operation = match.Groups["Operation"].Value;
					string operand = match.Groups["Operand"].Value;
					string compareRegisterName = match.Groups["CompareRegisterName"].Value;
					string comparison = match.Groups["Comparison"].Value;
					string comparand = match.Groups["Comparand"].Value;

					if (comparisons[comparison](GetRegister(compareRegisterName), int.Parse(comparand)))
					{
						int newValue = registers[alterRegisterName] = operations[operation](GetRegister(alterRegisterName), int.Parse(operand));

						maximumEver = Math.Max(maximumEver, newValue);
					}
				}
			}

			Console.WriteLine(maximumEver);
		}
	}
}
