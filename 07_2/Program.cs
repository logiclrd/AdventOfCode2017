...
wkvfo (177) -> muaiipe, nctisdd, tdyqxhp";
		...
		{
			var programByName = new Dictionary<string, ProgramInfo>();

			ProgramInfo GetProgram(string name)
			{
				ProgramInfo ret;

				if (!programByName.TryGetValue(name, out ret))
					programByName[name] = ret = new ProgramInfo();
			...
			{
				var parser = new Regex(@"^(?<ProgramName>\w+) \((?<Weight>\d+)\)( -\> (?<ReferredTo>\w+)(, (?<ReferredTo>\w+))*)?$", RegexOptions.Singleline);
				...
				while (true)
				{
					...
					string programName = match.Groups["ProgramName"].Value;

					var program = GetProgram(programName);

					program.Name = programName;
					program.OwnWeight = int.Parse(match.Groups["Weight"].Value);
					program.Carrying.AddRange(match.Groups["ReferredTo"].Captures.OfType<Capture>().Select(cap => GetProgram(cap.Value)));
					program.Carrying.ForEach(p => p.IsCarried = true);
				}
			}

			var rootProgram = programByName.Values.Single(program => !program.IsCarried);

			CheckWeights(rootProgram);
		}

		private static void CheckWeights(ProgramInfo rootProgram)
		{
			foreach (var carrying in rootProgram.Carrying)
				CheckWeights(carrying);

			var weightGroups = rootProgram.Carrying.GroupBy(p => p.TotalWeight).OrderBy(grouping => grouping.Count()).ToList();

			if (weightGroups.Count > 1)
			{
				var outlier = weightGroups.First();
				var expected = weightGroups.Last();

				Console.WriteLine("Programs with incorrect weights:");

				foreach (var p in outlier)
				{
					Console.WriteLine("* " + p.Name + " => actual weight " + p.OwnWeight);
					Console.WriteLine("* " + p.Name + " => actual total weight " + p.TotalWeight);
				}

				Console.WriteLine("Weight of other programs on the same disc: " + expected.First().TotalWeight);

				foreach (var p in outlier)
				{
					var delta = expected.First().TotalWeight - p.TotalWeight;

					Console.WriteLine("* " + p.Name + " difference: " + delta);

					p.OwnWeight += delta;
					p.TotalWeight += delta;

					Console.WriteLine("* " + p.Name + " should be: " + p.OwnWeight);
				}
			}

			rootProgram.TotalWeight = rootProgram.OwnWeight + rootProgram.Carrying.Sum(p => p.TotalWeight);
		}
	}
}
