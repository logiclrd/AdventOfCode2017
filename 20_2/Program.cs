...
p=<-2954,-985,167>, v=<-418,-140,23>, a=<29,7,-3>";
		...
		static void Main(string[] args)
		{
			...
			// Technically, the input *could* have particles that collide after 100,000 iterations...
			for (int iteration = 0; iteration < 100000; iteration++)
			{
				Console.Write('.');

				var positionSet = new Dictionary<(long X, long Y, long Z), int>();

				for (int i = 0; i < particles.Count; i++)
				{
					var particle = particles[i];

					if (!positionSet.TryGetValue(particle.Position, out int count))
						positionSet[particle.Position] = 1;
					else
						positionSet[particle.Position] = count + 1;
				}

				for (int i = particles.Count - 1; i >= 0; i--)
				{
					if (positionSet[particles[i].Position] > 1)
					{
						Console.WriteLine("REMOVED: {0}", particles[i].Position);
